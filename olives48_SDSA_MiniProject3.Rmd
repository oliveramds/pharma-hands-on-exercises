...
![Demographic table](img/demog_summary_table.PNG).
...
varSummary <- function(dataframe, summary_variable) {
  dataframe %>%
    group_by(TRT01AN, TRT01A, SEX) %>%
    summarize(mean = mean({{summary_variable}}) %>% round(digits = 1) %>% format(nsmall=1)  ,
              sd = sd({{summary_variable}}) %>% round(digits = 1) %>% format(nsmall = 1), 
              med = median({{summary_variable}}) %>% round(digits = 1) %>% format(nsmall = 1),              
              min = min({{summary_variable}}) %>% round(digits = 1) %>% format(nsmall = 1), 
              max = max({{summary_variable}}) %>% round(digits = 1) %>% format(nsmall = 1),
              n = n()) %>%
    mutate(range_minmax= paste0("(",min, ",", max, ")"))

}

height = varSummary(adsl_eff, HEIGHTBL) %>%
  ungroup() %>%
  select("TRT01A","SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","SEX"), names_to ="Category", values_to = "values" ) %>%
...
weight = varSummary(adsl_eff, WEIGHTBL) %>%
  ungroup() %>%
  select("TRT01A","SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","SEX"), names_to ="Category", values_to = "values" ) %>%
