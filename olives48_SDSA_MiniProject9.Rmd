...
BICARB_plot <- adlb %>%  
    filter(SAFFL == "Y", PARAMCD == "BASO") %>%
    ggplot(mapping = aes(x = ADY, y = AVAL)) + 
    geom_point(aes(colour = "THERAPY1"))

BICARB_plot 
#Code doesn't work when copy pasting and then reprexing afterwards, so putting code into reprex instead, copying this into console below.
