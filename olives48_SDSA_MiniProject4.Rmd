...
![Demographic table](img/MiniProject4_demog_table.png). After completing the challenge, the updated table should be similar to this one: ![Demographic table](img/demog_summary_table.PNG)
...
```{r}
dm_allcomb_1 = age_cat %>%
  ungroup() %>%
  bind_rows(agestat_cat) %>%
  dplyr::mutate(category = factor(category,
                    levels = c("<65", "65-80", ">80", "N", "Mean", "Median", "Std Dev", "Range(min,max)"))) %>%
  arrange(category)

```
2.  Move N before Age- categories.
```{r}
dm_allcomb_2 = age_cat %>%
  ungroup() %>%
  bind_rows(agestat_cat) %>%
  dplyr::mutate(category = factor(category,
...
```{r}
big_N_ethnicity = adsl_eff %>%
  group_by(TRT01AN, TRT01A, ETHNIC) %>%
  count(name = "N")

small_n_cnt_ethnicity =  adsl_eff %>%
  group_by( TRT01AN, TRT01A, SEX, ETHNIC) %>%
  count(name = "n")

ethnicity_merge_count <- small_n_cnt_ethnicity %>% 
  left_join(big_N_ethnicity, 
            by = c("TRT01AN", "TRT01A", "ETHNIC")) %>%
  ungroup()

ethnicity_data = ethnicity_merge_count  %>% 
  mutate(perc_tot = round((n/N)*100, 1))

ethnicity_data_2 <- ethnicity_data %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1))

ethnicity_n_percent = ethnicity_data_2 %>%
...
small_n_cnt_race =  adsl_eff %>%
  group_by( TRT01AN, TRT01A, SEX, RACE) %>%
  count(name = "n")

race_merge_count <- small_n_cnt_race %>% 
  left_join(big_N_race, 
            by = c("TRT01AN", "TRT01A", "RACE")) %>%
  ungroup()

race_data = race_merge_count  %>% 
  mutate(perc_tot = round((n/N)*100, 1))

race_data_2 <- race_data %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1))

race_n_percent = race_data_2 %>%
  mutate(npct = paste(n, paste0("(", perc_tchar, ")"))) %>% 
  select(RACE, TRT01A, SEX, npct)

final_ethnicity = pivot_wider(ethnicity_n_percent, 
                              names_from = c(TRT01A,SEX), 
                              values_from = npct, 
                              values_fill = "0",
                              names_sep = "_") %>%
...
final_race = pivot_wider(race_n_percent, 
                              names_from = c(TRT01A,SEX), 
                              values_from = npct, 
                              values_fill = "0",
                              names_sep = "_") %>%
...
adsl_complete = bind_rows(dm_allcomb_2, final_ethnicity, final_race) %>%
  dplyr::mutate(category = factor(category,
                    levels = c("N", "<65", "65-80", ">80", "Mean", "Median", "Std Dev", "Range(min,max)", 
