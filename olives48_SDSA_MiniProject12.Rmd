...
adsl_counts <- function(adsl_data) {
  adsl_mrg_cnt <- calculate_small_n_cnt(adsl_data) %>%
    left_join(calculate_Big_N_cnt(adsl_data), by = c("TRT01A", "TRT01AN")) %>%
    mutate(perc = round((n/N)*100, digits=1)) %>%
    ungroup() %>%
    select(TRT01A, SEX, perc) 
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
   test_small_n_cnt <- calculate_small_n_cnt(adsl_data)
   expect_named(test_small_n_cnt, c("TRT01AN", "TRT01A", "SEX", "n"))
   expect_equal(dim(test_small_n_cnt), c(length(test_small_n_cnt$TRT01A), 4))
})
...
test_that("adsl_counts() output has expected dim, columns, and types", {
  ...
  expect_equal(dim(adsl_test_data), 
               c(length(unique(adsl_test_data$TRT01A))*length(unique(adsl_test_data$SEX)), 3))
  expect_type(adsl_test_data$perc, "double")
})

test_that("percentage calc is correct", {
  aggregate_Big_N_cnt <- test_data %>% 
    group_by(TRT01A) %>%
    count(name = "N")
  
  aggregate_small_n_cnt <- test_data %>%
    group_by(TRT01A, SEX) %>% 
    count(name = "n")
  
  merged_perc <- aggregate_small_n_cnt %>%
    left_join(aggregate_Big_N_cnt, by = c("TRT01A")) %>%
    mutate(perc = round((n/N)*100, digits=1)) %>%
    ungroup() %>%
    select(TRT01A, SEX, perc)
  
  test_adsl_count <- adsl_counts(test_data, Big_N_cnt, small_n_cnt)
  
  expect_identical(merged_perc$perc, test_adsl_count$perc)
})
