...
PR <- ADVS %>%
  filter(PARAMN == 3) %>% 
  filter(VISITNUM > 3) %>%
  mutate(StudyWeek = floor(ADY/7)) #The floor() function in R returns the highest number that is less than or equal to a number set as a parameter
...
```{r}
plot2 <- plot1 + 
  labs(title = "Spaghetti plot showing heart rate by subject over relative analysis days",
       subtitle = "Protocol number",
       x = "Analysis Relative Day",
       y = "Heart Rate",
...
```{r}
plot4 <- plot3 +
  facet_wrap(facets = "TRTA") #cab also facet_wrap(. ~)
plot4
# 
# plot4 <- plot3 +
#   facet_grid(rows = "TRTA") #cab also facet_wrap(. ~)
# plot4
```
...
xLab <- quo_name(DBP_plot$mapping$x)
yLab <- quo_name(DBP_plot$mapping$y)
groupLab <- quo_name(DBP_plot$mapping$group)

study <- uniqueVal(DBP_plot$data$STUDYID)
measure <- uniqueVal(DBP_plot$data$PARAM)
...
xLab_temp <- quo_name(TEMP_plot$mapping$x)
yLab_temp <- quo_name(TEMP_plot$mapping$y)
groupLab_temp<- quo_name(TEMP_plot$mapping$group)

study <- uniqueVal(TEMP_plot$data$STUDYID)
measure <- uniqueVal(TEMP_plot$data$PARAM)


xLab_temp <- ifelse(xLab_temp == "ADY", "Analysis Relative Day", xLab_temp)
groupLab_temp <- case_when(groupLab_temp == "USUBJID" ~ "Subject", 
                      groupLab_temp == "TRTA" ~ "Treatment",
                      TRUE ~ groupLab_temp)
...
```{r}
ADVS_2 <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/advsmax.xpt")

Pulse_Rate <- ADVS_2  %>%
  filter(PARAMCD == "PULSE") %>%
  filter(SAFFL   == "Y")

plot5 <- ggplot(data = Pulse_Rate, mapping = aes(x= ADY, y =AVAL, group = USUBJID)) +
         geom_line()
#plot5

plot6 <- plot5 +
         labs(title = "Vital Signs across time",
              subtitle = "Pulse Rate",
              x = "Study Day",
...
Pulse_Rate_plot <- plot6 %+%
  Pulse_Rate

xLab <- quo_name(Pulse_Rate_plot$mapping$x)
yLab <- quo_name(Pulse_Rate_plot$mapping$y)
groupLab <- quo_name(Pulse_Rate_plot$mapping$group)

study <- uniqueVal(Pulse_Rate_plot$data$STUDYID)
measure <- uniqueVal(Pulse_Rate_plot$data$PARAM)

xLab <- ifelse(xLab == "ADY", "Study Day", xLab)
groupLab <- case_when(groupLab == "SUBJID" ~ "Subject", 
                      groupLab == "TRTP" ~ "Treatment",
                      TRUE ~ groupLab)
